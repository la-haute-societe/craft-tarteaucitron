...
module.exports = config => {
  let copyFilesArray = getCopyFilesPathsArray(
    config.assets.copy,
    config.outputDir
  );
  ...
  return {
    ...
    output: {
      ...
      filename: `${config.assets.scripts}/${
        config.filenames[process.env.NODE_ENV].js
      }`
    },
    ...
    plugins: [
      ...
      new MiniCssExtractPlugin({
        filename: `${config.assets.styles}/${
          config.filenames[process.env.NODE_ENV].css
        }`,
        chunkFilename: `${config.assets.styles}/${
          config.filenames[process.env.NODE_ENV].cssChunk
        }`
      })
    ],
